        ...
        <GlassCard variant="gold" className="lg:col-span-2 h-[600px] flex flex-col justify-between p-0 overflow-hidden">
            ...
            {chatMessages.map(msg => (
              ...
              >
                ...
                <div className={`max-w-xl space-y-2 ${msg.sender === 'user' ? 'items-end' : 'items-start'}`}>
                  {/* Text Bubble */}
                  <div
                    className={`p-4 rounded-control text-xs leading-relaxed font-sans ${
                      msg.sender === 'user'
                  ...
                  {msg.sender === 'ai' && msg.sources && (
                    <div className="p-3.5 rounded-control bg-surface/90 border border-surface-border text-xs space-y-2.5 shadow-sm">
                      ...
                      <div className="space-y-1">
                        ...
                        {msg.sources.map((src, sIdx) => (
                          <div key={sIdx} className="text-[11px] font-sans text-primary flex items-center gap-1.5 bg-surface p-2 rounded-control border border-surface-border">
                      ...
                      <div className="flex flex-wrap items-center gap-2 pt-1">
                        <button
                          ...
                          className="px-3 py-1 rounded-control bg-surface hover:bg-surface-subtle text-status-info-text font-sans text-[10px] font-medium flex items-center gap-1 border border-surface-border"
                        >
                          <Share2 size={10} /> View Graph
                        </button>
                        <button
                          onClick={() => handleSaveInsight(msg.text)}
                          className="px-3 py-1 rounded-control bg-accent/10 hover:bg-accent/20 text-primary font-sans text-[10px] font-medium flex items-center gap-1 border border-accent/30"
                        >
          ...
          <form onSubmit={handleSend} className="p-3 bg-surface/90 border-t border-surface-border flex items-center gap-2">
            <input
              ...
              className="flex-1 bg-surface border border-surface-border focus:border-accent/40 rounded-control px-4 py-2.5 text-xs text-primary placeholder-[#A8A29E] focus:outline-none font-sans"
            />
            <button
              type="submit"
              className="p-2.5 rounded-control bg-surface-soft hover:bg-surface-subtle text-primary font-medium shadow-sm transition-all"
            >
            ...
            <div className="space-y-2 text-xs font-sans">
              {[
                ...
                <button
                  ...
                  className="w-full text-left p-3 rounded-control bg-surface/80 hover:bg-surface border border-surface-border hover:border-accent text-primary transition-all text-xs font-sans leading-snug"
                >
                  "{q}"
                </button>
              ))}
            ...
            {savedInsights.length > 0 ? (
              <div className="space-y-2">
                {savedInsights.map((insight, idx) => (
                  <div key={idx} className="p-3 rounded-control bg-surface/80 border border-surface-border text-xs text-primary font-sans">
